...
void stella_overflow(void);

struct stella_timetable_entry* current = 0;

volatile uint16_t stella_pwm_compare = 0;
volatile uint16_t stella_pwm_counter = 0;

ISR(STELLA_COMPARE_VECTOR) {
    if ((stella_pwm_counter == stella_pwm_compare) && current) {
        ACCESS_IO(current->port.port) |= current->port.mask;
        current = current->next;

        if (current) {
        	stella_pwm_compare = current->value;
        }
	}
    stella_pwm_counter++;
    if (stella_pwm_counter > 1023) {
            stella_pwm_counter = 0;
            stella_overflow();
    }
}
...
void stella_overflow(void) {
    ...
    ACCESS_IO(int_table->port[0].port) = (ACCESS_IO(int_table->port[0].port) & ~(uint8_t)stella_portmask[0]) | int_table->port[0].mask;
